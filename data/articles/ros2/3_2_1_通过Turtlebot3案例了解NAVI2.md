...
tags:
- ROS2智能车
thumbnail: /api/v1/website/image/ros2/2_ros2_navi2_demo_thumbnail.jpg
...
![IP地址查询](/api/v1/website/image/ros2/2_get_ip_address.png)

- 2. 修改 `sudo gedit /etc/hosts`文件,并手动添加 DNS 解析：

![手动添加IP](/api/v1/website/image/ros2/2_add_ip_into_linux_host.png)
...
![ROS2 小乌龟例子](/api/v1/website/image/ros2/2_ros2_turtlesim_demo.png)
...
![gazebo模型仓库](/api/v1/website/image/ros2/2_gazebo_models_repo.png)

5. 解压 zip 文件夹并重命名为`models`，然后拷贝到`~/.gazebo`的目录下

![移动gazebo模型](/api/v1/website/image/ros2/2_put_gazebo_models.png)
...
![启动trutlebot3的gazebo物理环境](/api/v1/website/image/ros2/2_run_gazebo.png)
...
![建图的rviz2画面](/api/v1/website/image/ros2/2_gazebo_rviz2_map.png)
...
![控制机器人运动生成大概的地图文件](/api/v1/website/image/ros2/2_generate_map.png)
...
![启动RVIZ导航](/api/v1/website/image/ros2/2_run_navi.png)

3. 给出起始点：在`rviz2`工具上拖动`2D Pose Estimate`，给出小车位置，完成坐标的变换：

![RVIZ2启动导航仿真](/api/v1/website/image/ros2/2_navi_demo.gif)
